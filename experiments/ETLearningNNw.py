...
class Diffusion(Module):
    def __init__(self, nvec):
        ...
        self.nvec = nvec
        self.nmod = len(nvec)
        self.W = Parameter(torch.tensor(np.random.rand(self.nmod, self.nmod)))
        self.mask = None

    def forward(self, U):
        nvec = torch.tensor(self.nvec, device=self.W.device)
        W = torch.repeat_interleave(torch.repeat_interleave(self.W, nvec, dim=0), nvec, dim=1)
        Wtril = torch.tril(W)
