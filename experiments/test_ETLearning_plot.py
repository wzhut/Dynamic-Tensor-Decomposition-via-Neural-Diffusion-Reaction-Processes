...
if __name__ == '__main__':
    ...
    model = ETL(nvec, dim_embedding, 100, [dim_embedding+1, 20, 20, 20, dim_embedding], torch.device('cuda:{}'.format(cuda)))
    ...
    for i in range(4):
        for j in range(6):
            for k in range(6):
                for l in range(6):
                    batch_idx = np.tile([i, j, k, l], [100, 1])
                    batch_T = time.reshape((-1, 1))
                    pred_m, pred_std = model.predict_np(batch_idx, batch_T)
                    
                    tr_sel = (ind[:, 0] == i) & (ind[:, 1] == j) & (ind[:, 2] == k) & (ind[:, 3] == l)
                    tr_tp = t[tr_sel]
                    tr_yp = y[tr_sel]

                    te_sel = (ind_te[:, 0] == i) & (ind_te[:, 1] == j) & (ind_te[:, 2] == k) & (ind_te[:, 3]==l)
                    te_tp = t_te[te_sel]
                    te_yp = y_te[te_sel]
